            ...
            <table class="w-full text-left">
               <thead class="bg-theme-bg border-b border-theme-border">
                  <tr>
                     ...
                     <th class="px-6 py-3 text-[10.5px] font-bold text-theme-text-muted uppercase tracking-widest">Agent</th>
                     <th class="px-6 py-3 text-[10.5px] font-bold text-theme-text-muted uppercase tracking-widest">Activity</th>
               ...
               <tbody class="divide-y divide-theme-border">
                  ...
                  <tr class="hover:bg-theme-bg transition-colors">
                     <td class="px-6 py-4 text-[13.5px] font-bold text-theme-text-main">{{ $session->client->user->name ?? 'Unknown' }}</td>
                     ...
                     <td class="px-6 py-4">
                        @if($session->status === 'Active')
                           <span class="px-2 py-0.5 rounded-full bg-green-100 text-green-600 text-[10.5px] font-bold">Active</span>
                        @else
                           <span class="px-2 py-0.5 rounded-full bg-gray-100 text-gray-600 text-[10.5px] font-bold">Completed</span>
                           @if($session->report_summary)
                           <div class="mt-1">
                              <button data-title="{{ $session->activity_name }}" data-summary="{{ $session->report_summary }}" data-participation="{{ $session->report_participation_level }}" data-outcome="{{ $session->report_outcome_notes }}" data-recommendations="{{ $session->report_follow_up_recommendations }}" onclick="viewReport(this)" class="text-[#1a3cdc] text-[11px] font-bold hover:underline cursor-pointer">View Report</button>
                           </div>
                           @endif
                        @endif
                     </td>
                  ...
                  @empty
                  <tr>
                     <td colspan="5" class="px-6 py-4 text-center text-[13px] text-theme-text-muted">No sessions found.</td>
                  </tr>
                  @endforelse
               </tbody>
            </table>
   ...
   <div>
      <div class="bg-slate-900 rounded-[14px] p-6 text-white shadow-xl relative overflow-hidden">
         ...
         <div class="flex gap-2">
            <button onclick="openStopSessionModal({{ $activeSession->id }})" class="flex-1 py-2.5 bg-[#1a3cdc] rounded-lg text-[12px] font-bold hover:bg-[#1230b0]">Stop Session & File Report</button>
         </div>
...
<div id="stopSessionModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
   <div class="bg-theme-card w-full max-w-lg rounded-[14px] shadow-2xl border border-theme-border overflow-hidden max-h-[90vh] overflow-y-auto">
      <div class="px-6 py-4 border-b border-theme-border flex items-center justify-between">
         <h3 class="text-[16px] font-extrabold text-theme-text-main">File Activity Report & Stop Session</h3>
         <button onclick="document.getElementById('stopSessionModal').classList.add('hidden')" class="text-theme-text-muted hover:text-theme-text-main transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
         </button>
      </div>
      <div class="p-6">
         <form id="stopSessionForm">
            ...
            <div class="space-y-4">
               <div>
                  <label class="block text-[12px] font-bold text-theme-text-main mb-1">Summary</label>
                  <textarea name="report_summary" required rows="3" placeholder="Brief summary of the activity..." class="w-full bg-theme-bg border border-theme-border rounded-[8px] px-3 py-2 text-[13px] text-theme-text-main focus:outline-none focus:border-[#1a3cdc]"></textarea>
               </div>
               <div>
                  <label class="block text-[12px] font-bold text-theme-text-main mb-1">Participation Level</label>
                  <select name="report_participation_level" required class="w-full bg-theme-bg border border-theme-border rounded-[8px] px-3 py-2 text-[13px] text-theme-text-main focus:outline-none focus:border-[#1a3cdc]">
               ...
               <div>
                  <label class="block text-[12px] font-bold text-theme-text-main mb-1">Outcome / Progress Notes</label>
                  <textarea name="report_outcome_notes" required rows="3" placeholder="Notes on client progress or outcomes..." class="w-full bg-theme-bg border border-theme-border rounded-[8px] px-3 py-2 text-[13px] text-theme-text-main focus:outline-none focus:border-[#1a3cdc]"></textarea>
               </div>
               <div>
                  <label class="block text-[12px] font-bold text-theme-text-main mb-1">Follow-up Recommendations</label>
                  <textarea name="report_follow_up_recommendations" rows="3" placeholder="Any follow-ups required?" class="w-full bg-theme-bg border border-theme-border rounded-[8px] px-3 py-2 text-[13px] text-theme-text-main focus:outline-none focus:border-[#1a3cdc]"></textarea>
               </div>
            </div>
            <div class="mt-6 flex justify-end gap-3">
               <button type="button" onclick="document.getElementById('stopSessionModal').classList.add('hidden')" class="px-4 py-2 bg-theme-bg text-theme-text-main border border-theme-border rounded-[8px] text-[13px] font-bold hover:bg-theme-border transition-colors">Cancel</button>
               <button type="submit" class="px-4 py-2 bg-[#1a3cdc] text-white rounded-[8px] text-[13px] font-bold hover:bg-[#1230b0] transition-colors">Submit Report & Stop</button>
            </div>
         </form>
      </div>
   </div>
...
<!-- New Session Modal -->
<div id="newSessionModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
   <div class="bg-theme-card w-full max-w-lg rounded-[14px] shadow-2xl border border-theme-border overflow-hidden">
      <div class="px-6 py-4 border-b border-theme-border flex items-center justify-between">
         ...
         <button onclick="document.getElementById('newSessionModal').classList.add('hidden')" class="text-theme-text-muted hover:text-theme-text-main transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
         </button>
      </div>
      <div class="p-6">
         <form id="newSessionForm">
            ...
            <div class="space-y-4">
               <div>
                  ...
                  <select name="client_id" required class="w-full bg-theme-bg border border-theme-border rounded-[8px] px-3 py-2 text-[13px] text-theme-text-main focus:outline-none focus:border-[#1a3cdc]">
               ...
               <div>
                  ...
                  <input type="text" name="activity_name" required placeholder="e.g. Morning Walk" class="w-full bg-theme-bg border border-theme-border rounded-[8px] px-3 py-2 text-[13px] text-theme-text-main focus:outline-none focus:border-[#1a3cdc]">
               </div>
               <div>
                  <label class="block text-[12px] font-bold text-theme-text-main mb-1">Activity Type</label>
                  <select name="activity_type" required class="w-full bg-theme-bg border border-theme-border rounded-[8px] px-3 py-2 text-[13px] text-theme-text-main focus:outline-none focus:border-[#1a3cdc]">
               ...
               <div>
                  ...
                  <input type="text" name="location" placeholder="e.g. Central Park" class="w-full bg-theme-bg border border-theme-border rounded-[8px] px-3 py-2 text-[13px] text-theme-text-main focus:outline-none focus:border-[#1a3cdc]">
               </div>
               <div>
                  <label class="block text-[12px] font-bold text-theme-text-main mb-1">Notes/Goals</label>
                  <textarea name="notes" rows="3" placeholder="Any specific goals for this session?" class="w-full bg-theme-bg border border-theme-border rounded-[8px] px-3 py-2 text-[13px] text-theme-text-main focus:outline-none focus:border-[#1a3cdc]"></textarea>
               </div>
            </div>
            <div class="mt-6 flex justify-end gap-3">
               <button type="button" onclick="document.getElementById('newSessionModal').classList.add('hidden')" class="px-4 py-2 bg-theme-bg text-theme-text-main border border-theme-border rounded-[8px] text-[13px] font-bold hover:bg-theme-border transition-colors">Cancel</button>
               <button type="submit" class="px-4 py-2 bg-[#1a3cdc] text-white rounded-[8px] text-[13px] font-bold hover:bg-[#1230b0] transition-colors">Start Session</button>
            </div>
         </form>
      </div>
   </div>
