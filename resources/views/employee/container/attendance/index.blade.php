...
@if(!isset($error))
<div class="grid sm:grid-cols-1 xm:grid-cols-1 grid-cols-3 gap-8">
            ...
            <div class="bg-white p-4 rounded-lg border border-red-100 flex items-center justify-between">
               <div>
                  <div class="text-[13px] font-bold text-slate-800">
                     {{ $missed->serviceBooking->client->user->name ?? 'Unknown Client' }}</div>
                  <div class="text-[12px] text-slate-500">
                     Check-in: {{ $missed->check_in->format('M d, Y h:i A') }}
                  </div>
               </div>
               <button onclick="openMissedPunchModal({{ $missed->id }})"
                  class="px-4 py-2 bg-red-600 text-white rounded-lg text-[12px] font-bold hover:bg-red-700">Fix
                  Punch</button>
            </div>
            @endforeach
         </div>
            ...
            <div class="p-6">
               <div class="flex items-center justify-between mb-4">
                  <div>
                     <div class="text-[15px] font-bold text-slate-800">
                        {{ $booking->client->user->name ?? $booking->patient_name }}</div>
                     <div class="text-[13px] text-slate-500 mt-0.5">{{ $booking->service->name ?? 'Service' }} |
                        {{ $booking->address }}, {{ $booking->city }}</div>
                  </div>
                  <div>
                     @if(!$attendance)
                     <button onclick="checkIn({{ $booking->id }})"
                        class="px-5 py-2.5 bg-green-600 text-white rounded-lg text-[13px] font-bold shadow-md hover:bg-green-700 transition-all flex items-center gap-2">
                     ...
                     <button onclick="openCheckOutModal({{ $attendance->id }})"
                        class="px-5 py-2.5 bg-red-600 text-white rounded-lg text-[13px] font-bold shadow-md hover:bg-red-700 transition-all flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                           <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                     ...
                     @else
                     <span
                        class="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg text-[12px] font-bold">Completed</span>
                     @endif
                  </div>
               </div>
               ...
               <div class="bg-slate-50 rounded-lg p-4 grid grid-cols-2 gap-4">
                  <div>
                     ...
                     <div class="text-[13px] font-bold text-slate-800">{{ $attendance->check_in->format('h:i A') }}
                     </div>
                  </div>
                  <div>
                     <div class="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Check Out</div>
                     <div class="text-[13px] font-bold text-slate-800">
                        {{ $attendance->check_out ? $attendance->check_out->format('h:i A') : '--:--' }}</div>
                  </div>
               </div>
               @endif
            </div>
      ...
      <div class="bg-slate-900 rounded-[14px] p-6 text-white shadow-xl">
         ...
         <div class="space-y-4 mb-8">
            <div>
               ...
               <div class="text-4xl font-extrabold text-[#1a3cdc]">{{ $totalHoursThisWeek }}<span
                     class="text-xl text-white/50">h</span></div>
            </div>
            <div>
         ...
         <form id="checkOutForm">
            ...
            <div class="space-y-4">
               <div>
                  ...
                  <textarea name="note" rows="3" placeholder="Any issues or notes during this visit?"
                     class="w-full border border-slate-200 rounded-lg px-4 py-2 text-[13px] focus:outline-none focus:border-[#1a3cdc] focus:ring-1 focus:ring-[#1a3cdc]"></textarea>
               </div>
            </div>
            <div class="mt-6 flex gap-3 justify-end">
               <button type="button" onclick="document.getElementById('checkOutModal').classList.add('hidden')"
                  class="px-5 py-2.5 rounded-lg text-[13px] font-bold text-slate-600 hover:bg-slate-50 border border-slate-200">Cancel</button>
               <button type="submit"
                  class="px-5 py-2.5 bg-red-600 text-white rounded-lg text-[13px] font-bold shadow-md hover:bg-red-700">Submit
                  Check Out</button>
            </div>
         </form>
      </div>
...
<!-- Missed Punch Modal -->
<div id="missedPunchModal"
   class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
   <div class="bg-white w-full max-w-md rounded-[14px] shadow-2xl overflow-hidden">
      <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
         <h3 class="text-[16px] font-extrabold text-slate-800">Fix Missed Punch</h3>
         <button onclick="document.getElementById('missedPunchModal').classList.add('hidden')"
            class="text-slate-400 hover:text-slate-600">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
               <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
            </svg>
      ...
      <div class="p-6">
         <form method="POST" id="missedPunchForm">
            @csrf
            <div class="space-y-4">
               <p class="text-[13px] text-slate-600">Submit a correction request for this visit. Admin will review and
                  adjust your timesheet.</p>
               <div>
                  <label class="block text-[13px] font-bold text-slate-700 mb-2">Explanation / Correct Time</label>
                  <textarea name="note" required rows="3" placeholder="e.g. I left at 4:30 PM but forgot to check out."
                     class="w-full border border-slate-200 rounded-lg px-4 py-2 text-[13px] focus:outline-none focus:border-[#1a3cdc] focus:ring-1 focus:ring-[#1a3cdc]"></textarea>
               </div>
            </div>
            <div class="mt-6 flex gap-3 justify-end">
               <button type="button" onclick="document.getElementById('missedPunchModal').classList.add('hidden')"
                  class="px-5 py-2.5 rounded-lg text-[13px] font-bold text-slate-600 hover:bg-slate-50 border border-slate-200">Cancel</button>
               <button type="submit"
                  class="px-5 py-2.5 bg-slate-900 text-white rounded-lg text-[13px] font-bold shadow-md hover:bg-slate-800">Submit
                  Request</button>
            </div>
         </form>
      </div>
         ...
         navigator.geolocation.getCurrentPosition(
            ...
            (error) => {
               ...
               resolve({
                  lat: null,
                  lng: null
               }); // Proceed even if denied as per requirements
            }, {
               timeout: 5000
            }
         );
