...
@section('admin-content')
    <div class="w-full flex items-center justify-between gap-5">
        <div>
            <div class="text-2xl font-extrabold text-slate-800">Attendance Management</div>
        ...
        <div class="flex gap-3">
            <button
                class="px-5 py-2.5 bg-white border border-[#1a3cdc] text-[#1a3cdc] rounded-[10px] text-[13px] font-bold hover:bg-[#eef2ff] transition-all">Late/Off
                Alert</button>
            <button
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 my-5">
        <div class="bg-white rounded-[14px] p-5 border border-slate-200 shadow-sm">
            <div class="text-slate-400 text-[12px] font-bold uppercase tracking-widest mb-1">Present Today</div>
            <div class="text-2xl font-extrabold text-slate-800">28</div>
            <div class="mt-2 flex items-center">
                <span class="px-2 py-0.5 rounded-full bg-green-100 text-green-600 text-[10.5px] font-bold">On Duty</span>
            </div>
        </div>
        <div class="bg-white rounded-[14px] p-5 border border-slate-200 shadow-sm">
            <div class="text-slate-400 text-[12px] font-bold uppercase tracking-widest mb-1">Late Today</div>
        ...
        <div class="bg-white rounded-[14px] p-5 border border-slate-200 shadow-sm">
            <div class="text-slate-400 text-[12px] font-bold uppercase tracking-widest mb-1">Absent Today</div>
        ...
        <div class="bg-white rounded-[14px] p-5 border border-slate-200 shadow-sm">
            ...
            <div class="text-2xl font-extrabold text-slate-800">2</div>
            <div class="mt-2 flex items-center">
                <span class="px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 text-[10.5px] font-bold">Approved</span>
            </div>
        </div>
    </div>
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div class="lg:col-span-2">
            <div class="bg-white rounded-[14px] border border-slate-200 overflow-hidden shadow-sm">
                <div class="px-6 py-5 border-b border-slate-200 flex items-center justify-between">
                    <h3 class="text-[15px] font-extrabold text-slate-800">Today's Attendance — Oct 23, 2023</h3>
                    <select
                        class="bg-slate-50 border border-slate-200 rounded-[8px] px-3 py-1.5 text-[12px] font-bold outline-none">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        <thead class="bg-slate-50 border-b border-slate-200">
                            <tr>
                                <th class="px-6 py-3 text-[10.5px] font-bold text-slate-400 uppercase tracking-widest">Agent
                                </th>
                                <th class="px-6 py-3 text-[10.5px] font-bold text-slate-400 uppercase tracking-widest">
                                    Check-In</th>
                                <th class="px-6 py-3 text-[10.5px] font-bold text-slate-400 uppercase tracking-widest">
                                    Check-Out</th>
                                <th class="px-6 py-3 text-[10.5px] font-bold text-slate-400 uppercase tracking-widest">Hours
                                </th>
                                <th class="px-6 py-3 text-[10.5px] font-bold text-slate-400 uppercase tracking-widest">
                                    Status</th>
                                <th
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            ...
                                            JW</div>
                                        <div class="text-[13px] font-bold text-slate-700">James Wilson</div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-[13px] text-slate-600">08:02 AM</td>
                                <td class="px-6 py-4 text-[13px] text-slate-600">05:00 PM</td>
                                <td class="px-6 py-4 text-[13px] text-slate-600 font-bold">8.9 hrs</td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            ...
                                            LB</div>
                                        <div class="text-[13px] font-bold text-slate-700">Lisa Brown</div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-[13px] text-slate-600">08:31 AM</td>
                                <td class="px-6 py-4 text-[13px] text-slate-400">—</td>
                                <td class="px-6 py-4 text-[13px] text-slate-400">—</td>
                                <td class="px-6 py-4"><span
        ...
        <div>
            <div class="bg-white rounded-[14px] border border-slate-200 p-6 shadow-sm">
                <h3 class="text-[14.5px] font-bold text-slate-800 mb-6">October 2023 — Monthly Overview</h3>
                <div class="grid grid-cols-7 gap-2">
                    @foreach(['M', 'T', 'W', 'T', 'F', 'S', 'S'] as $d)
                        <div class="text-center text-[10px] font-bold text-slate-400 mb-1">{{ $d }}</div>
                    @endforeach
                    @for($i = 1; $i <= 21; $i++)
                        <div
                            class="aspect-square rounded-lg flex flex-col items-center justify-center gap-0.5 {{ $i % 7 == 0 || ($i + 1) % 7 == 0 ? 'bg-slate-50 text-slate-400' : ($i == 10 ? 'bg-red-50 text-red-600' : ($i == 4 ? 'bg-amber-50 text-amber-600' : 'bg-green-50 text-green-600')) }}">
