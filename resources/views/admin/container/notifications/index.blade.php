...
@endif
<div class="grid sm:grid-cols-1 xm:grid-cols-1 grid-cols-2 gap-5 my-5">
      ...
      <div class="bg-theme-card rounded-[14px] border border-theme-border overflow-hidden shadow-sm">
         <div class="px-6 py-5 border-b border-theme-border flex items-center justify-between">
            ...
            <form action="{{ route('notifications.mark-all-as-read') }}" method="POST">
               @csrf
               <button type="submit" class="text-[12px] font-bold text-[#1a3cdc] hover:underline">Mark all as
                  read</button>
            </form>
            @endif
         </div>
         <div class="divide-y divide-theme-border">
            ...
            <div
               class="p-6 flex gap-4 hover:bg-theme-bg transition-colors {{ is_null($notification->read_at) ? 'bg-blue-50/20' : '' }}">
               <div
                  class="w-10 h-10 rounded-full bg-theme-primary-light text-theme-primary flex items-center justify-center flex-shrink-0">
               ...
               <div class="flex-1">
                  <div class="flex items-center justify-between mb-1">
                     <span
                        class="text-[14px] font-bold text-theme-text-main {{ is_null($notification->read_at) ? 'text-theme-primary' : '' }}">{{ $notification->data['title'] ?? 'Notification' }}</span>
                     <span
                        class="text-[11px] text-theme-text-muted">{{ $notification->created_at->diffForHumans() }}</span>
                  </div>
                  <p class="text-[13px] text-theme-text-muted leading-relaxed">
                  ...
                  <div class="mt-3 flex gap-2">
                     <form action="{{ route('notifications.mark-as-read', $notification->id) }}" method="POST">
                        @csrf
                        <button type="submit"
                           class="px-3 py-1 bg-[#1a3cdc] text-white rounded-[6px] text-[11px] font-bold">Mark as
                           Read</button>
                     </form>
                  </div>
                  @endif
      ...
      <div class="bg-theme-card rounded-[14px] border border-theme-border overflow-hidden shadow-sm mt-8">
         ...
         <div class="divide-y divide-theme-border">
            ...
            <div class="p-6 flex gap-4 hover:bg-theme-bg transition-colors">
               ...
               <div class="flex-1">
                  <div class="flex items-center justify-between mb-1">
                     <span class="text-[14px] font-bold text-theme-text-main">Broadcast to
                        {{ $broadcast->audience }}</span>
                     <span
                        class="text-[11px] text-theme-text-muted">{{ $broadcast->created_at->diffForHumans() }}</span>
                  </div>
                  <p class="text-[13px] text-theme-text-muted leading-relaxed">{{ $broadcast->message }}</p>
                  <div class="mt-2 text-[11px] text-theme-text-muted font-bold">Sent by:
                     {{ $broadcast->sender?->name ?? 'Admin' }}</div>
               </div>
            </div>
            @empty
