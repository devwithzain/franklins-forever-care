<aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
   ...
   <div class="p-6 pb-2">
      <div>
         <img class="w-80 h-auto object-cover block dark:hidden" src="{{ asset('assets/logo.png') }}" alt="Logo">
         <img class="w-80 h-auto object-cover hidden dark:block" src="{{ asset('assets/logoDark.png') }}" alt="Logo">
      </div>
   </div>
      ...
      <div class="mb-8">
         ...
         <div class="space-y-1">
            ...
            <a href="{{ route('admin.contact-submissions.index') }}"
               ...
               @php
               $newInquiriesCount = \App\Models\ContactSubmission::where('status',
               \App\Models\ContactSubmission::STATUS_NEW)->count();
               @endphp
               @if($newInquiriesCount > 0)
               <span
                  class="px-1.5 py-0.5 rounded-full bg-blue-100 text-blue-600 text-[9px] font-extrabold">{{ $newInquiriesCount }}</span>
               @endif
            </a>
            <a href="{{ route('admin.newsletter.index') }}"
               ...
               @php
               $pendingSubscribersCount = \App\Models\NewsletterSubscriber::where('status',
               \App\Models\NewsletterSubscriber::STATUS_PENDING)->count();
               @endphp
               @if($pendingSubscribersCount > 0)
               <span
                  class="px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-600 text-[9px] font-extrabold">{{ $pendingSubscribersCount }}</span>
               @endif
            </a>
         </div>
      </div>
